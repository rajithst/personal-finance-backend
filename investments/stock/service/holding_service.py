...
from investments.stock.models import Holding, StockSplit
from investments.stock.serializers import HoldingSerializer, ResponseHoldingSerializer
            ...
            if is_dev_env:
                ...
                for portfolio in portfolios:
                    taskqueue.add(
                        ...
                        method='GET',
                        url='/investments/stocks/holdings/refresh/',
...
class HoldingService:
    ...
    def refresh_holdings_with_current_price(self, request_data):
        try:
            ...
            for holding in queryset:
                ...
                Holding.cron_objects.filter(id=holding.id).update(current_price=current_price,
    ...
    def get_stock_splits(self, companies=None):
        stock_split_history = StockSplit.cron_objects.select_related('company').filter(
            company_id__in=companies).order_by(
