...
def mock_portfolio_validator(mocker):
    return mocker.patch('investments.apis.views.PortfolioValidator')

@pytest.mark.django_db
class TestInvestmentPerformanceView:
    def test_get_investment_performance_success(self, api_client, mock_dashboard_service, mock_portfolio_validator, authenticate):
        # Arrange
        authenticate()
        ...
        mock_dashboard_service.return_value.get_passive_income.return_value = 500
    ...
    def test_get_investment_performance_validation_error(self, api_client, mock_portfolio_validator, authenticate):
        # Arrange
        authenticate()
        mock_portfolio_validator.return_value.validate_request.side_effect = Exception('Validation Error')
    ...
    def test_get_investment_performance_service_error(self, api_client, mock_dashboard_service, mock_portfolio_validator, authenticate):
        # Arrange
        authenticate()
        mock_portfolio_validator.return_value.validate_request.return_value = None
