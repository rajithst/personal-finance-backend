...
class CategorySettingsView(APIView):
    def put(self, request):
        try:
            ...
            if updated_category:
                return Response({'data': {'category': updated_category, 'subcategories': all_subcategories}, 'status': True, 'message': 'Success'}, status=status.HTTP_200_OK)
            return Response({'data': {'category':None, 'subcategories': None}, 'status': False, 'message': 'Error'}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({'data': {'category':None, 'subcategories': None}, 'status': False, 'message': str(e)}, status=status.HTTP_400_BAD_REQUEST)
